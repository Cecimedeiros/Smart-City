...
* **Limite de Reconexão do Redis:** A configuração do cliente Redis define um número máximo de 3 tentativas de reconexão (`maxRetriesPerRequest`). Isso impede que falhas pontuais no servidor Redis causem ciclos infinitos de reconexão que exaurem os recursos do processo Node.js.
...
* **Atualizações Otimistas no Frontend:** Ao alterar o status ou a prioridade de uma demanda, a interface atualiza imediatamente o estado local (Zustand) sem aguardar a confirmação da API. Isso elimina o atraso perceptível para o gestor durante interações, tornando a experiência mais fluida sem comprometer a consistência — em caso de erro da API, o estado é revertido.
